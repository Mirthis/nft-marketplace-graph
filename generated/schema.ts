...
export class ItemSold extends Entity {
  ...
  save(): void {
    ...
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type ItemSold must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("ItemSold", id.toString(), this);
    }
  }

  static load(id: string): ItemSold | null {
    return changetype<ItemSold | null>(store.get("ItemSold", id));
  }
